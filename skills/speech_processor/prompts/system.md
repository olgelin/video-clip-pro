...
4. **注入知识增强**：在说话人观点框架内，用你的知识储备补充——
   - 相关数据（如"算力成本在过去两年下降了X%"）；优先用**跨国家/跨制度/跨时间对比**的一组数据坐实判断，不是单点罗列
   - 行业案例（如"就像XX公司做的那样"）
...
7. **框架命名**：把说话人的核心观点/底层逻辑，提炼成一个**能命名的机制或概念**。
   优先学科术语 + 大白话翻译（如"这在心理学上叫「防御性关系退缩」，大白话就是……"），
   其次自创可记忆的名字（如"时间贫困""信息平权"）。
...
- 第1段：钩子——用颠覆认知 + 情绪冲击直接抓住注意力（戳穿一个被普遍接受的说法 + 强烈情绪张力），不要温和代入
- 中间段：展开论证——每段一层意思，层层递进
- 最后1段：金句升华；同时**每一段都要有至少 1 句可截图、可转发的金句**，全文金句密度要高，不是只在结尾憋一句
