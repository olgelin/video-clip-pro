...
- 🔴 **底部字幕安全区（内容绝对禁入）**：画面底部 **150px（横屏）/ 180px（竖屏）** 是底部字幕的位置。**任何元素**——数据卡片、图表、标签、图例、注脚、底部装饰文字、小数字、副标题——**一律禁止进入这个区域**。⚠️ **不是容器 `bottom:150px` 就行**：文字字体高度会向下溢出，`justify-content:space-between` 时最底部子元素会贴到容器 bottom 再往下溢出字体高度。所以**容器 bottom 必须 ≥180px（横屏）/ 200px（竖屏）**，给最底部一行文字留足 30px 字体余量。哪怕只是一个"婚礼平均成本 ¥xx万"这样的小注脚标签，放到底部就会和字幕重叠、文字打架。内容全部集中在字幕区之上，底部留干净。
