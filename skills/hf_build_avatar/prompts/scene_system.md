...
### inset = 缩小的小画面（聚焦）
内容**不铺满**，缩成一个 40-60% 大小的"小画面"（一个集中的信息块/卡片/窗口），四周留白。观众视线被逼着聚焦它。

🔴 inset 小画面怎么"活"起来（不是缩一块就完事）——从下面 3 个形态里自由选 1 个：
1. **居中小窗**：小画面居中，入场 scale 0.6→1 + 从下方/上方滑入，入场后呼吸缩放 scale 1.0↔1.04
2. **角落缩位**：小画面缩到左上/右下角，留出大片空白给氛围粒子/隐喻物体，缓慢漂移
3. **悬浮漂移**：小画面在画布上持续缓慢移动 + 缩放（translate 1-2px/s + scale 呼吸），像漂浮

🔴 inset 铁律：
- 四周留白是"设计"，不是"没排满"——留白本身就是画面的力量
...
- 🔴 **留白锚**：每屏至少保留一个空象限（左上/右上/左下/右下，至少一个角是空的）。四角全占满 = 画心空洞，判凌乱。
