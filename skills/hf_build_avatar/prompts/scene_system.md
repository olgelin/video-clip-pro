...
画面里有一个 `data-person-zone` 虚线占位框，那是**数字人出镜的位置**（最终会被真人数字人视频覆盖）。你的标题/卡片/数据/图表必须**全部避开这个占位框**，排在占位框之外：
...
🔴 **禁止把标题、卡片、数据、图表放进占位框区域**——那里是数字人的位置，会被盖住。你的可排版区域 = 画面减去人物区后的剩余空间，在这个空间内做满排版（垂直填满、不留大片空白）。这是同层排版，不是"内容铺满 + 数字人叠上面"。
