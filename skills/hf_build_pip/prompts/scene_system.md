...
- 🔴 **同屏主体 ≤3 个**：数字人（画中画）算 1 个主体，所以你的内容主体最多 2 个（比如 1 个大数字 + 1 组卡片）。超过 3 个主体 = 画面乱。
- 🔴 **垂直填满（抄 video-factory）**：内容必须在安全区内**从上到下、从左到右铺满**，上下左右都不留大片空白。上半屏标题、中半屏核心数据/图表、下半屏卡片/趋势线，三层都要有内容。如果内容偏少，用更大字号、更多数据卡片、更宽的间距来填满。🔴 **禁止"居中一小块四周空"**（内容缩在画面中央一小块、四周大片空白 = 空，判不合格）。
...
- 🔴 90% 安全区：竖屏 1080×1920 左右 54px、上下 96px；横屏 1920×1080 左右 96px、上下 54px。垂直填满不留大片空白
...
- 🔴 **辅助文字最小字号铁律**：任何标签/说明/注脚/图例/单位/小字，**最小 22px**。禁止写 12-18px 的小字——小于 22px 的辅助文字在手机上完全看不清，等于废字。宁可少放一个辅助信息，也不要放看不清的小字。
